...
.PHONY: build-windows
build-windows: # Building mac binaries
	@echo Building ${TAG} binary for windows
	GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build -ldflags="-X 'main.BuildVersion=${TAG}'" -o bin/server ./cmd
...
update-docker-tag: # Update the docker tag
	...
	yq e -i '.spec.template.spec.containers[0].image="spirosoik/argo:${TAG}"' deployments/argo/rollout.yaml
